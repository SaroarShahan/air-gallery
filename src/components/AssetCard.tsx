...
const AssetCard = ({ clip }: CardProps) => {
  const [isHovered, setIsHovered] = useState(false);

  const classes = clsx(
    `group relative overflow-hidden before:absolute before:left-0 before:top-0 before:h-full before:w-full before:bg-gradient-to-b before:from-transparent before:to-gray-800 inset-[-6px] rounded-md overflow-hidden border-4 border-transparent hover:border-gray-200 transition-all break-inside-void mb-4`
  );
